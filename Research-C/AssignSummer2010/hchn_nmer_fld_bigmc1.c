...
int argc; char *argv[];
{
	...
	double g0, g1, gcont, diff, SIGMA2;
	...
	double r, r2, r2_min, r2_max, r2_avg,Rg_avg,Rg_var,r2_var;
...
/*-----------------------Chain moves----------------*/
			...
			}/* end of else.. If statement of the choice*/
		}/*end of 1 MC*/
...
/*-----------Now the calculation after each accumulation cycle of each block---------- */
		if (ncyc%NACCUM==0) {      /*--take data every NACCUM cycle-*/
			...
			xdel=fabs(x[NCHN+N-1]-x[N]); ydel=fabs(y[NCHN+N-1]-y[N]); zdel=fabs(z[NCHN+N-1]-z[N]);
        
		        if(xdel>Lx/2.0){ xdel=Lx-xdel;}
        	        if(ydel>Ly/2.0){ ydel=Ly-ydel;}
        	        if(zdel>Lz/2.0){ zdel=Lz-zdel;}
